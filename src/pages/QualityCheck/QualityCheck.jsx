      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">
            Material quality inspection and grading
          </p>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-sky-100 bg-sky-50/50">
                ...
                <th className="px-5 py-4 text-left text-xs font-semibold text-slate-500 uppercase">Batch</th>
                <th className="px-5 py-4 text-left text-xs font-semibold text-slate-500 uppercase">Inspector</th>
                <th className="px-5 py-4 text-left text-xs font-semibold text-slate-500 uppercase">Grade</th>
                <th className="px-5 py-4 text-left text-xs font-semibold text-slate-500 uppercase">Date</th>
                <th className="px-5 py-4 text-left text-xs font-semibold text-slate-500 uppercase">Result</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {sortedData.map((row) => (
                <tr key={row.id} className="hover:bg-sky-50/30 transition-colors">
                  ...
                  <td className="px-5 py-3.5">
                    <span className={`inline-flex items-center justify-center w-8 h-8 rounded-lg text-xs font-bold ${row.grade === 'A+' || row.grade === 'A'
                  ...
                  <td className="px-5 py-3.5">
                    <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium border ${row.result === 'Passed'
        ...
        {sortedData.map((row) => (
          <div key={row.id} className="bg-white rounded-2xl p-4 border border-sky-100 shadow-sm card-hover">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium border ${row.result === 'Passed' ? 'bg-emerald-50 text-emerald-600 border-emerald-200' :
        ...
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
          ...
          <div className="bg-white rounded-2xl p-6 w-[340px] shadow-xl">
            ...
            <p className="text-sm text-slate-500 mb-4">
              {selectedRow?.id} - {selectedRow?.material}
            </p>
